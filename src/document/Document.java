...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		for(int k = 0; k < cArray.length; k++) {
			if (k == cArray.length - 1 && Character.toLowerCase(cArray[k]) == 'e') {
				
			}
			else if(newString && vowels.indexOf(Character.toLowerCase(cArray[k])) != -1) {
				CountVowels++;
				newString = false;
			}
			if(!newString && vowels.indexOf(Character.toLowerCase(cArray[k])) == -1){
				newString = true;
			}
		}
